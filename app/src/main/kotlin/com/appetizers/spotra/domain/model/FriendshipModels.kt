...
data class FriendProfile(
    ...
    val initials: String get() = initialsOf(firstName, lastName)
...
fun initialsOf(firstName: String, lastName: String): String = buildString {
    firstName.firstOrNull()?.let { append(it.uppercaseChar()) }
    lastName.firstOrNull()?.let { append(it.uppercaseChar()) }
}.ifBlank { "?" }

fun shortDisplayName(firstName: String, lastName: String): String {
    val trimmedFirst = firstName.trim()
    val lastInitial = lastName.firstOrNull()?.uppercaseChar()
    return if (lastInitial != null) "$trimmedFirst $lastInitial." else trimmedFirst
}
