# Meeting 01 – Team Formation & Project Planning

**Date:** May 21, 2026  
**Time:** 6:00 PM  
**Location:** Google Meet

## Attendees

- Akshat Jawne
- Eric Zhu
- Vraj Bhavsar
- Vishvam Patel
- Raghav Verma
- Edmond Yang

## Meeting Agenda

1. Team introductions
2. Discuss project idea
5. Assign preliminary team roles
6. Establish communication channels
7. Determine recurring meeting schedule
8. Complete P1 requirements
9. Plan out P3 Project Proposal Work

---

## Discussion Notes

### Selected Project Direction

Brief description of the chosen project:
App-etizers is a mobile platform designed to help students discover and evaluate study spaces on and around campus. The app provides an interactive map that displays study locations along with community-generated insights such as noise level, lighting quality, Wi-Fi reliability, occupancy levels, and average session duration. Students can filter locations based on their preferences, including solo study or group study environments, and quickly access relevant resources such as University of Waterloo room booking pages.

To encourage engagement, users can rate and review study spaces after their sessions, contributing to a continuously updated database of study spot information. The platform also incorporates social features that allow students to see when friends or connections are studying nearby, connect with other students interested in group study sessions, and discover popular study locations within the community.

To further enhance the user experience, the app may include gamification elements such as badges, leaderboards for top-rated study spaces, and an interactive map experience that rewards exploration of new study locations. The overall goal is to help students find productive study environments while fostering a stronger sense of community and collaboration on campus.

We have also completed P1 together and planned out the work for P3 - Project Proposal. 

---

## Team Roles

Role & Member
- Team Lead -> Akshat Jawne 
- Technical Lead -> Vraj Bhavsar
- Design Leads -> Vishvam Patel, Edmond Yang
- Back-End Leads -> Eric Zhu, Raghav Verma
  
---

## Communication

**Primary Platform**
- Discord

---

## Meeting Schedule

**Recurring Meeting Time**
- Weekly every Thursday @ 6:00 PM

---

## Decisions Made

1. Repository created on GitHub.
2. Team contract will be maintained in `/docs/team-contract.md`.
3. Meeting notes will be stored in `/docs/meetings/`.

---

## Action Items

1. Create repository structure
2. Complete P1 - Project Setup
3. Planned out the work to do betwee the group for P3: Project Proposal. 

---
